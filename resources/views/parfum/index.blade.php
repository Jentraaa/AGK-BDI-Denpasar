...
<head>
    ...
    <style>
        ...
        /* Logika Margin untuk desktop saat sidebar terbuka */
        .sidebar-open #main-content { margin-left: 18rem; }
...
<body class="bg-[#F8F8F8] text-[#1A1A1A] font-[Inter] antialiased sidebar-closed">
    ...
    <div class="flex min-h-screen">
        ...
        <main id="main-content" class="flex-1 ml-0">
                ...
                <div id="archive-section">
                    <header class="mb-20 flex flex-col md:flex-row justify-between items-start md:items-end gap-10">
                        <div class="flex-1">
                            ...
                            <h2 class="text-5xl lg:text-6xl font-serif text-gray-900 leading-tight">Oil <span class="italic font-normal">Products</span></h2>
                        </div>
                        
                        <div class="w-full md:w-80 group">
                            <form action="/#archive-section" method="GET" class="relative">
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Find brand or name..." class="w-full bg-transparent border-b border-gray-200 py-3 pr-10 text-sm focus:outline-none focus:border-[#C9A14A] transition-all duration-500 placeholder:text-gray-300 placeholder:italic font-serif">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-10 gap-y-20">
                        ...
                        <div class="group bg-white p-4 border border-transparent hover:border-gray-100 hover:shadow-2xl transition-all duration-700">
                            <div class="relative overflow-hidden bg-[#FBFBFB] mb-8 aspect-square flex items-center justify-center">
                                @if($p['image'])
                                    <img src="{{ asset('storage/'.$p['image']) }}" class="w-full h-full object-cover transform group-hover:scale-110 transition duration-[1.5s] ease-out">
                                @else
                                ...
                                <div class="absolute inset-0 bg-white/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500 backdrop-blur-[2px] flex items-center justify-center">
                                    <a href="/parfum/{{$p['id']}}" class="bg-black text-white px-8 py-3 text-[10px] uppercase tracking-[0.3em] font-bold hover:bg-[#C9A14A] transition-all transform translate-y-8 group-hover:translate-y-0 duration-500">View Detail</a>
                                </div>
                            </div>

                            <div class="space-y-3 text-center">
                                <p class="text-[9px] tracking-[0.4em] text-[#C9A14A] uppercase font-bold">{{ $p['brand'] }}</p>
                                <h3 class="text-xl font-serif text-gray-800 leading-tight group-hover:text-[#C9A14A] transition-colors">{{ $p['name'] }}</h3>
                                <div class="w-8 h-[1px] bg-gray-200 mx-auto my-4"></div>
                                <p class="text-[13px] text-gray-400 leading-relaxed font-light line-clamp-2 px-4">"{{ $p['description'] }}"</p>
                        ...
                        @empty
                        <div class="col-span-full py-40 text-center">Empty Vault.</div>
                        @endforelse
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        window.addEventListener('load', () => {
            // Memastikan sidebar tertutup saat load tanpa menghapus logika lainnya
